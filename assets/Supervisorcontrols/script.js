...
//      if(id.value==savedtext[index] || id.value.length==0) {
    ...
    function updatelabel(result){
       ...
       // Check for errors
       if(result.error) {
         submittedboxid.innerHTML="Error: "+ result.error.message[0].description; 
       } else {
         submittedboxid.innerHTML="Successfully updated " + result.description + " to " + result.defaultValue;
       }
    }    
  }
